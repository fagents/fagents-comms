...
class TestAgentAvailability:
    """Tests for /api/agents/available endpoint (cooperation: who's around?)."""

    def test_available_returns_agents(self, server_info):
        """GET /api/agents/available returns list of agents."""
        url, token, name = server_info
        s, data = _raw_request(url, token, "GET", "/api/agents/available")
        assert s == 200
        assert "agents" in data
        names = [a["name"] for a in data["agents"]]
        assert name in names

    def test_ai_agent_offline_no_health(self, server_info):
        """AI agent with no health data shows as offline."""
        url, token, name = server_info
        _server_module.AGENT_HEALTH.pop(name, None)
        s, data = _raw_request(url, token, "GET", "/api/agents/available")
        agent = next(a for a in data["agents"] if a["name"] == name)
        assert agent["type"] == "ai"
        assert agent["availability"] == "offline"

    def test_ai_agent_online_with_health(self, server_info):
        """AI agent with recent health report shows as online."""
        url, token, name = server_info
        import time as _time
        _server_module.AGENT_HEALTH[name] = {
            "context_pct": 42, "reported_at": _time.time(),
        }
        s, data = _raw_request(url, token, "GET", "/api/agents/available")
        agent = next(a for a in data["agents"] if a["name"] == name)
        assert agent["availability"] == "online"
        assert agent["context_pct"] == 42

    def test_human_no_timezone_unknown(self, server_info):
        """Human agent without timezone shows unknown availability."""
        url, token, name = server_info
        profiles = _server_module.load_agent_profiles()
    ...
    def test_human_with_timezone(self, server_info):
        """Human agent with timezone gets computed availability."""
        url, token, name = server_info
        profiles = _server_module.load_agent_profiles()
        profiles[name] = {"type": "human", "timezone": "UTC"}
        _server_module.save_agent_profiles(profiles)
        s, data = _raw_request(url, token, "GET", "/api/agents/available")
        agent = next(a for a in data["agents"] if a["name"] == name)
        assert agent["type"] == "human"
        assert agent["availability"] in ("available", "away")
        assert "local_time" in agent

    def test_includes_role_and_status(self, server_info):
        """Available endpoint includes role and status from profile."""
        url, token, name = server_info
        profiles = _server_module.load_agent_profiles()
        profiles[name] = {"type": "human", "role": "Ops lead", "status": "In meeting"}
        _server_module.save_agent_profiles(profiles)
        s, data = _raw_request(url, token, "GET", "/api/agents/available")
        agent = next(a for a in data["agents"] if a["name"] == name)
        assert agent["role"] == "Ops lead"
        assert agent["status"] == "In meeting"

    def test_invalid_timezone_returns_unknown(self, server_info):
        """Human with invalid timezone shows unknown, not error."""
        url, token, name = server_info
        profiles = _server_module.load_agent_profiles()
        profiles[name] = {"type": "human", "timezone": "Not/A/Timezone"}
        _server_module.save_agent_profiles(profiles)
        s, data = _raw_request(url, token, "GET", "/api/agents/available")
        assert s == 200
        agent = next(a for a in data["agents"] if a["name"] == name)
